...
class SmsPollingService : Service() {
    ...
    private fun stopPolling() { running = false; handler.removeCallbacks(poll); getSharedPreferences("nana_sms", MODE_PRIVATE).edit().putBoolean("running", false).apply(); stopForeground(STOP_FOREGROUND_REMOVE); stopSelf() }
